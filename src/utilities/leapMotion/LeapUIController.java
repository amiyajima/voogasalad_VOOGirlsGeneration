...
/**
 * GUI controller to configure leap motion setup for key and mouse and saves the
 * configuration to a properties file to be read in by the LeapMotionListener
 * listener.
 */
public class LeapUIController extends Application {
    ...
    /**
     * method to set up the available gesture types for key binding checking for
     * the already chosen gestures in the mouse clicking dropdown.
     */
    ...
    /**
     * to set mouse tracking of leap motion allows for dynamic UI dropdown based
     * on the current available options
     */
    protected void setUpDropDown () {
        ...
        ClickDropDown.valueProperty().addListener(event -> {
            setUpGesture();
            onSelect();
        });
    ...
    /**
     * opens a file saver; saves the map to a properties file and exits upon
     * clicking the button
     **/
    @FXML
