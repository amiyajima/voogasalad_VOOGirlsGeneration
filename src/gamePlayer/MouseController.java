...
/**
 * Contains Methods Responsible for Handling the Action of a Mouse Button Being
 * Clicked
 * 
 * @author
 *
...
public class MouseController {
	...
	public void setOnClick(ViewController vc, IGridState gridState,
			GameGrid grid) {
	...
	public void performAction(ViewController vc, double x, double y) {
		...
		myGridState.onClick(vc.getGame().getCurrentLevel().getGrid()
				.getPiece(vc.findPosition(x, y)));
	}
	...
	// myCurrentNode = node;
	//
	// myPreviousLocation = new
	// Point2D.Double(GridPane.getRowIndex(myPreviousNode),GridPane.getColumnIndex(myPreviousNode));
	// myCurrentLocation = new
	// Point2D.Double(GridPane.getRowIndex(myCurrentNode),
	// GridPane.getColumnIndex(myCurrentNode));
	//
	// vc.highlightCurrentLocation(Color.GREEN, myPreviousLocation,
	// myCurrentLocation);
	// }
	// });
	// }
